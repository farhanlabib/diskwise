...
export const jetbrainsProfile: AppProfile = {
  ...
  rationale:
    'JetBrains IDEs keep only indexes and local history in ~/Library/Caches/JetBrains, which each IDE rebuilds by re-indexing the projects it opens. Your projects, settings and plugins live under ~/Library/Application Support/JetBrains, which is protected. Android Studio keeps its caches under ~/Library/Caches/Google/AndroidStudio*, but that is a glob and diskwise profiles list exact paths, so it is deliberately left out; Android Studio re-indexes the same way if you clear that folder by hand.',
  regeneration:
