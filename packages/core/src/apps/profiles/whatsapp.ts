...
export const whatsappProfile: AppProfile = {
  ...
  rationale:
    'WhatsApp stores your chats and the media in them in its shared group container, which is protected. That data is handled separately by diskwise’s Tier 2 WhatsApp media rule, which moves it to the Trash only on request; here only the app-level cache folder is offered.',
  regeneration:
