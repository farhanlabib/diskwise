...
macsweep ships without a paid Apple Developer account, so the helper is signed
...
succeeds. The product is copied to `bin/macsweep-helper`, signed ad-hoc, and
...
1. `$MACSWEEP_HELPER`
2. `packages/native-helper/bin/macsweep-helper` relative to the module
3. `macsweep-helper` next to `process.argv[1]` (i.e. beside the installed CLI)
